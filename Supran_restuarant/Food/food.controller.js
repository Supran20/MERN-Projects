...
router.delete("/food/delete/:id", isValidAdmin, async (req, res) => {
  ...
  // send res
  return res.status(200).send({ message: "Food item is deleted successfully" });
});
...
router.get("/food/detail/:id", isValidAdmin, async (req, res) => {
  ...
  //if not food item, throw error
  if (!foodItem) {
    return res.status(404).send({ message: "Food item doesnt exist" });
  }
  //send res
  return res.status(200).send({ messgae: "success", foodDetail: foodItem });
});
